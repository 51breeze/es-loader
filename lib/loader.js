            ...
            const done =( error )=>{
                if( error ){
                    ...
                    if( filesystem.existsSync( resourceFile ) ){
                        ...
                        var content = filesystem.readFileSync( resourceFile );
                        if( this.mode ==="development" && options.hot && !compilation.isDescriptionType && !isStyle ){
                            content = `/*${Math.random()}*/\r\n${content}\r\n${getHotContent(compilation, options)}`;
                        }

                        if( options.client.options.babel && !isStyle  && !compilation.isDescriptionType ){
                            const babelOps = typeof options.client.options.babel === 'object' ? options.client.options.babel : {};
                            babelOps.babelrc = babelOps.babelrc !== false;
                            babel.transformAsync(content.toString(), babelOps).then( (result)=>{
