...
function normalizePath(compilation, file, resource, query={}){
    if( query.vue == void 0){
        if(query.type==='style'){
           return compiler.normalizeModuleFile(compilation,null,'style',query.file)
        }
        return compiler.normalizePath(resource)
    }
    return compiler.normalizePath(file);
}
            ...
            if( errors && errors.length > 0 ){
                ...
                if( content ){
                    ...
                    if( options.hot && plugins && !compilation.isDescriptionType && !(query && query.type === 'style') ){
                        ...
                        let sourceMap = builder.getGeneratedSourceMapByFile(resourceFile);
                        if(query && query.type === 'style' && query.file && options.compileStyle!==false){
