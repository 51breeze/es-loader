    ...
    if( compilation ){
        ...
        builder.build(compilation,(error)=>{
            ...
            if( errors && errors.length > 0 ){
                ...
                const resourceFile = compiler.normalizePath(file)
                let content = null;
                let sourceMap = null;
                if(query.type === 'style'){
                    let style = builder.getBuildStyle(resourceFile, query.index);
                    if(style){
                        content = style.content;
                    }
                }else{
                    let buildModule = builder.getBuildModule(resourceFile, query.id)
                    if(buildModule){
