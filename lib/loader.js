...
async function Loader(content){
    ...
    if(compilation){
        ...
        if(query.callhook != null && query.action){
            try{
                const content = await builder.callHook(compilation, query);
                const [pageDir] = builder.getRouteFiles();
                if(pageDir){
                    this.addContextDependency(path.normalize(pageDir));
                }
        ...
        let buildGraph = builder.getBuildGraph(compilation);
        if(buildGraph && !query.type){
            const dependFiles = buildGraph.getDependFiles();
            if(dependFiles && dependFiles.length>0){
                builder.clear(compilation);
            }
        }
        ...
        if(dependFiles && dependFiles.length>0){
            dependFiles.forEach(dependFile=>{
                const dir = path.normalize(dependFile.dir);
