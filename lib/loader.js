...
function getPlugin(config={}, context={}){
    ...
    let mode = context.mode || process.env.NODE_ENV;
    if(!config.options.mode && mode){
    ...
    if( Array.isArray(options.plugins) && options.plugins.length > 0 ){
        ...
        const build=(compilation, chnaged=false)=>{
            ...
            compilation.ready().then(()=>{
                ...
                let errors = errorHandle(context, compilation);
                if(errors && errors.length>0){
                    context.emitError( new Error( errors.join("\n") ) );
                    return;
                }
...
async function Loader(content){
    ...
    if( compilation ){
        ...
        if(!content){
            callback( new Error(`Build error code is empty. on the "${file}"`));
            return;
        }
