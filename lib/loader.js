...
function getHotContent(compilation, options){
    ...
    compilation.getDependencies().forEach( (depModule)=>{
        if( depModule.compilation !== compilation && !depModule.isDeclaratorModule && !depModule.compilation.isServerPolicy(depModule) ){
            deps.push(`'${getModuleFile(depModule)}'`);
        }
    });

    let hotCallback =`function(dependencyId){
        if( document ){   
            var moduleClass = __webpack_require__(dependencyId);    
            var event = document.createEvent('Event');
            event.initEvent('DEVELOPMENT_HOT_UPDATE', true, true);
            event.updatedClass = moduleClass.default || moduleClass;
    ...
    return `if(module.hot){
        ...
        module.hot.addStatusHandler(status => {
             ...
             if( status ==='ready'){
                module.hot.apply({onAccepted:(info)=>{
            ...
            const done =( error )=>{
                if( error ){
                    ...
                    if( filesystem.existsSync( this.resource ) ){
                        let content = filesystem.readFileSync( this.resource );
...
Loader.pitch = function(remainingRequest){
    const param = new URLSearchParams( this.resourceQuery );
    if( param.has('id') ){
        const options = loaderUtils.getOptions(this);
        const filesystem  = compiler.getOutputFileSystem( options.client.builder.name );
        if( filesystem.existsSync( remainingRequest ) ){
            const content = filesystem.readFileSync( remainingRequest );
            if( content ){
                return content;
            }
        }
